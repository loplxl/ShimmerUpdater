...
const STEPS: f32 = 8.0; //total steps to update
static mut STEPS_COMPLETE: u8 = 0;
const CREATE_NO_WINDOW: u32 = 0x08000000;
const DETACHED_PROCESS: u32 = 0x00000008;

fn taskkill(command:&str) -> Option<i32> {
    let cmd = Command::new("cmd")
        .args(["/C",command])
        .creation_flags(CREATE_NO_WINDOW) //create no window
        .output().expect(&format!("Failed to get output from command {}",command));
...
fn extract_shimmer(directory: &PathBuf, zipfile: fs::File) {
...

    {
        for i in 0..archive.len() {
            ...
            copy(&mut file, &mut output_file).expect("Failed to write to output file");
        }
    }
}
...
fn main() {
    ...
    let str_exe_code: i32 = taskkill("taskkill /f /im SetTimerResolution.exe").unwrap();
    stage(&format!("Killed SetTimerResolution.exe with exit code {}",str_exe_code));
    
    let internal_dir: PathBuf = software_dir.join("_internal");
    if internal_dir.exists() { fs::remove_dir_all(internal_dir).expect("Unable to remove _internal directory, maybe SetTimerResolution.exe is still running?"); }
    stage("Deleted _internal directory");
    ...
    stage("Extracted Shimmer.exe and _internal dir from Shimmer.zip");

    run_detached(software_dir.join("Shimmer.exe"));
    stage("Running Shimmer.exe");
    if str_exe_code == 0 {
